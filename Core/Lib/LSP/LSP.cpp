...
void lsp::SyntaxTreeViewParams::Deserialize(nlohmann::json json) {
    if (json["uri"].is_string()) {
        uri = json["uri"];
    }

    if (json["index"].is_number_integer()) {
        index = json["index"];
    }
}

nlohmann::json lsp::SyntaxTreeViewResponse::Serialize() {
    nlohmann::json array = nlohmann::json::array();

    for (auto &v: data) {
        auto object = nlohmann::json::object();
        object["label"] = v.label;
        object["index"] = v.index;
        object["range"] = v.range;
        object["isNode"] = v.isNode;
