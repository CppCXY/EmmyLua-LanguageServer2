	...
	for (int index = 2; index < argc; index++)
	{
		...
		if (current.starts_with("--"))
		{
			...
			if (eqPos != std::string_view::npos)
			{
				...
				{
					_errors.emplace_back(fmt::format("Unknown option {} ,please enable key=value options", current));
					return false;
				}
		...
		else if (current.starts_with("-"))
		{
			...
			{
				_errors.emplace_back(fmt::format("Unknown Option {}", current));
				return false;
			}
		...
		if (index < argc - 1)
		{
			...
			if(_argvs[index + 1].starts_with("-"))
			{
				...
				_errors.emplace_back(fmt::format("Option {} has not value", current));
				return false;
			}
...
void CommandLine::PrintUsage()
{
	...
	{
		std::cerr << fmt::format("{}:", target.first) << std::endl;
